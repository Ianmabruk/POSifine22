    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.auditLog.create({
        data: {
          ...
          entityType: 'Account',
          entityId: accountId,
          metadata: { reason, suspendedBy: (req as AuthRequest).user!.id },
        },
      });
    });
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.auditLog.create({
        data: {
          ...
          entityType: 'Account',
          entityId: accountId,
          metadata: { reactivatedBy: (req as AuthRequest).user!.id },
        },
      });
    });
...
router.post('/trials/extend', authenticateJWT, authorize(['MAIN_ADMIN']), validate(extendTrialSchema), async (req: AuthRequest, res, next) => {
  try {
    const { accountId, days, reason } = req.body;
    const adminId = req.user!.id;

    const trial = await prisma.trial.findUnique({ where: { accountId }, include: { account: true } });
    if (!trial) throw new AppError(404, 'TRIAL_NOT_FOUND', 'Trial not found for this account');
...
router.post('/payments/approve', authenticateJWT, authorize(['MAIN_ADMIN']), validate(approvePaymentSchema), async (req: AuthRequest, res, next) => {
  try {
    ...
    if (payment.paymentStatus !== 'PENDING') throw new AppError(400, 'PAYMENT_ALREADY_PROCESSED', 'Payment already processed');
...
router.post('/admins', authenticateJWT, authorize(['MAIN_ADMIN']), validate(createMainAdminSchema), async (req, res, next) => {
  try {
    ...
    if (existing) throw new AppError(409, 'EMAIL_TAKEN', 'Admin with this email already exists');
...
router.post('/admins/password-reset/request', async (req, res, next) => {
  try {
    ...
    if (!admin) return res.json(ok({ success: true }));
