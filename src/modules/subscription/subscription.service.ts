...
export class SubscriptionService {
  ...
  async getPlanByName(planName: string) {
    const plan = SubscriptionPlans[planName.toUpperCase() as keyof typeof SubscriptionPlans];
    if (!plan) {
  ...
  async createTrialSubscription(accountId: string, planName: string) {
  ...
  async getSubscriptionByAccount(accountId: string) {
  ...
  async expireTrial(subscriptionId: string) {
  ...
  async extendTrial(subscriptionId: string, days: number, extendedBy: string, reason: string) {
    const subscription = await prisma.subscription.findUnique({
      where: { id: subscriptionId },
      include: { account: true },
    });
  ...
  async convertTrialToSubscription(subscriptionId: string, billingCycle: 'monthly' | 'yearly') {
    const subscription = await prisma.subscription.findUnique({
      where: { id: subscriptionId },
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.account.update({
        ...
        data: {
          ...
          subscriptionPlan: subscription.plan.name as any,
  ...
  async rejectPayment(paymentId: string, rejectedBy: string, reason: string) {
    const payment = await prisma.payment.findUnique({
      where: { id: paymentId },
