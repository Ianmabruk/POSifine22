...
router.post('/trial', authenticateJWT, validate(createTrialSchema), async (req, res, next) => {
    try {
...
router.get('/subscription', authenticateJWT, async (req, res, next) => {
    try {
        const accountId = req.user.accountId;
...
router.post('/payment', authenticateJWT, validate(createPaymentSchema), async (req, res, next) => {
    try {
        const accountId = req.user.accountId;
...
router.get('/admin/stats', authenticateJWT, authorize(['MAIN_ADMIN']), async (_req, res, next) => {
    try {
...
router.get('/admin/businesses', authenticateJWT, authorize(['MAIN_ADMIN']), async (req, res, next) => {
    try {
...
router.get('/admin/trials/active', authenticateJWT, authorize(['MAIN_ADMIN']), async (_req, res, next) => {
    try {
...
router.get('/admin/trials/expired', authenticateJWT, authorize(['MAIN_ADMIN']), async (_req, res, next) => {
    try {
        const trials = await prisma.trial.findMany({
...
router.post('/admin/trials/extend', authenticateJWT, authorize(['MAIN_ADMIN']), validate(extendTrialSchema), async (req, res, next) => {
    try {
...
router.get('/admin/subscriptions', authenticateJWT, authorize(['MAIN_ADMIN']), async (req, res, next) => {
    try {
...
router.get('/admin/payments', authenticateJWT, authorize(['MAIN_ADMIN']), async (req, res, next) => {
    try {
        const { page = 1, limit = 20, status } = req.query;
        const skip = (Number(page) - 1) * Number(limit);
        const take = Number(limit);
...
router.post('/admin/payments/approve', authenticateJWT, authorize(['MAIN_ADMIN']), validate(approvePaymentSchema), async (req, res, next) => {
    try {
...
router.post('/admin/payments/reject', authenticateJWT, authorize(['MAIN_ADMIN']), validate(rejectPaymentSchema), async (req, res, next) => {
    try {
...
router.post('/admin/subscriptions/:id/suspend', authenticateJWT, authorize(['MAIN_ADMIN']), async (req, res, next) => {
    try {
...
router.post('/admin/subscriptions/:id/reactivate', authenticateJWT, authorize(['MAIN_ADMIN']), async (req, res, next) => {
    try {
...
router.get('/admin/revenue', authenticateJWT, authorize(['MAIN_ADMIN']), async (_req, res, next) => {
    try {
...
router.get('/admin/audit-logs', authenticateJWT, authorize(['MAIN_ADMIN']), async (req, res, next) => {
    try {
